...
function History() {
    ...
    const fetchSubmissionData = (order) => {
        axios.get(`http://localhost:5000/api/formSubmissions/reviewed`).then(res => {
            res.data.sort((a, b) => {
                if (order == "desc") {
    ...
    return (
        ...
        <Breadcrumb>
            ...
            <Breadcrumb.Item>Form Listing</Breadcrumb.Item>
        </Breadcrumb>
                ...
                <Row justify="left" align="stretch" gutter={[16,24]} style={{ display:'flex', flex: "1 1 0px", flexDirection:"row" }} key={refreshSubmission}>
                    { submissionData.length > 0 ? submissionData.map((submission, index) => {
                        return(
                            ...
                            extra={<a href={("/manager/history/" + submission.submission_id)}>Details</a>}
                            style={{ height:"100%" }}
                            bodyStyle={{
                                height:"80%"
                            }} >
                                <div style={{
                                    ...
                                    { submission.form_status == 1 ?
                                        <Tag icon={<CheckCircleOutlined />} color="success" style={{ marginTop: 10 }}>
                                            Approved
                                        </Tag>
                                    :
                                        <>
                                            <Tag icon={<CloseCircleOutlined />} color="error" style={{ marginTop: 10 }}>
                                                Rejected
                                            </Tag>
